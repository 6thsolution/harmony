...
extension TypeMethods on ConstantReader {
  ...
  bool get isRecursiveCollection {
    ...
    if (typeArg.isDartCoreIterable || typeArg.isDartCoreList || typeArg.isDartCoreSet || typeArg.isDartCoreMap) {
      return true;
    } else {
      return false;
    }
  ...
  List<DartType> get dartTypeArgs => objectValue.type?.typeArguments ?? <DartType>[];
  ...
  T enumValue<T>() {
    ...
    if (!classMirror.isEnum) {
      throw Exception('Could not read constant via enumValue<$T>(). $T is not a Dart enum.');
    }
